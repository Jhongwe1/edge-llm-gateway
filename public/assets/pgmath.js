...
(function () {
  ...
  /* 佔位符。三個地方都必須小心，2026-07-29 上線後炸過一次：
     1. **用跳脫序列寫，不要打字面字元**。初版寫的是字面 U+0001，存檔時被吃掉，
        PH_A 變成單一個 "M"、PH_B 變成空字串 —— 佔位符成了 M0、M1…M50。
     2. **前後都要有界標**。只有前綴的話 split("M5") 會把 "M50" 切成兩半：
        第 50 個公式被換成第 5 個公式的內容再接一個 "0"。
        實際災情：第 5 個公式是 x \to 5，於是畫面上出現 x → 50、x → 51、x → 52…
        整段解題過程變成無意義的數字，而前面幾條公式卻是正常的（i<10 時還沒撞上）。
     3. **替換要由後往前**（見 restore）—— 就算界標哪天又出事，長編號先被換掉，
        短編號也咬不到它。三層都獨立成立才叫安全。
     用 PUA（U+E000/E001）而不是控制字元：一樣不可能出現在正常文字或模型輸出裡，
     但它是普通可列印碼位，不會在任何一層工具鏈裡被「順手清掉」。 */
  var PH_A = "\uE000PGM";
  var PH_B = "\uE001";
  ...
  function restore(html, store) {
    ...
    // 由後往前：先換掉編號長的（M50），短的（M5）就不可能咬到它的前半段。
    // 界標已經能防這件事，這裡是第二層保險 —— 這個 bug 上線過一次，值得兩層。
