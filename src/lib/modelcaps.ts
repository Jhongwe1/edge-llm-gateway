...
//
// 3. **上游的數字只用來往下砍，不用來往上開。** 站上自己的 maxImgPerMsg 與
//    maxImgBytesTotal 跟上游肯收幾張是兩回事，所以一律取 min(自己的上限, 上游的上限)。
//    ⚠ 這段原本寫死「（4）與（1.5MB）」，那是 v2.4.1 當下的值，**現在是 10 與 16MB**
//    （2026-08 稽核抓到的漂移，DEBT #37）。**不要再把數字抄進註解** —— 唯一真相在
//    PG_LIMITS，而它的兩個數字有各自的來源：張數是「一次對話塞得下的合理量」，
//    bytes 才是量出來的護欄（組上游 body 的成本跟總 bytes 成正比、跟張數無關）。
//    註記：這條護欄的理由在 v2.6 之後也變了 —— 那筆花費現在發生在 30 秒 CPU 的
//    Durable Object 裡，不再是 Worker 的 10ms（ADR-0015），所以上限才放得寬。
//
// ── 目前已知會回報能力的上游 ──────────────────────────────────
