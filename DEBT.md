...
| 13 | ~~**串流貼著免費方案 10ms CPU 上限跑**（ADR-0011）~~ ✅ 2026-07-31 v2.5.0 **對 openai／custom 渠道還清**：改成原生直通（ADR-0014），線上實測 626ms → 6ms，CPU 不再跟串流長度成正比。**anthropic／gemini 仍走轉譯路徑，這條債對它們原封不動**（見 #28） | 站長拍板續用免費方案；當時兩層優化後 5982 增量約 4.2ms，看起來尚有餘裕 —— 事後證明那個量測只涵蓋解析側（總成本的 19.5%），真正的大頭在把位元組寫回瀏覽器 | ~~出現新的無聲截斷～→ 升 Workers Paid~~（2026-07-30 確實出現了：多圖多輪請求無聲截斷、`wrangler tail` 看到 `exceededCpu`；解法選了直通而不是付費方案） |
...
## 2026-07-31 v2.5.0 直通架構帶進來的（ADR-0014）
...
| # | 債 | 為何先欠 | 還債門檻 |
|---|---|---|---|
| 28 | **anthropic／gemini 渠道還在轉譯路徑上** —— 直通只認 OpenAI 相容形狀，這兩家的串流形狀不同，前端解不出來 | gemini 的 chunk 數量本來就比 OpenAI 少一個量級（實際流量 openai 69／gemini 17），撞上限的機率低很多；anthropic 目前無渠道 | 這兩種渠道也出現無聲截斷 → 要嘛前端補上這兩種形狀的解析（然後放進白名單），要嘛升 Workers Paid |
| 29 | **直通模式沒有「關掉網頁後在背景把回覆跑完」**（ADR-0012 的功能在這條路上不存在） | 換來的是「這種請求本來會被 CPU 殺掉、什麼都留不下」變成「活著跑完」；而且前端 `pagehide` 會用 `sendBeacon` 送出已收到的部分 | 會員實際回報「關了網頁回來只剩半截」→ 把生成搬進 Durable Object 自己管生命週期（同 #16 的解法） |
| 30 | **`sendBeacon` 的 64KB 本體上限** —— 關網頁那一刻若已生成超過 64KB，那一發送不出去（正常串完那條路沒有這個限制） | 瀏覽器的硬限制，繞不過；64KB 中文約 2 萬字，一般對話遠不到 | 出現「長回覆關網頁後整篇不見」的實際案例 → 改成串流期間每 N 秒回報一次（多幾次 D1 寫入換保險） |
| 31 | **直通模式的 token 數是「瀏覽器回報」的** —— 伺服器看不到 usage 訊框，只能信客戶端；只補得了一次、也只動得了自己的列，但數字本身可以被少報 | 配額執法算的是**請求次數**不是 token（ADR-0003），少報 token 佔不到便宜；受影響的只有成本報表的精確度 | 成本報表被實際拿來做決策（跟 #10／#25 同一個門檻） |
| 32 | **嗅探攔不到串流尾端才追加的欄位** —— 例：Venice 跟 usage 同一筆送出的 `cost:{usd,diem}`，會員打開開發者工具看得到 | 這是直通架構本身的邊界，不是檢查的疏漏（判斷時位元組還沒到）；而且程式碼早就寫明「隱藏上游不是安全邊界，真正的保護是會員拿不到 base_url 與金鑰」 | 站長認為這條曝光不可接受 → `pg_passthrough='0'` 一鍵退回轉譯，或改成不向上游要 `stream_options.include_usage`（代價是連 token 都沒有） |
