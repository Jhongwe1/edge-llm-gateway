...
| 測 Playground | `POST /api/playground/chat`（管理金鑰可直接測，SSE 串流）。**v2.5 起有兩種回應格式，看回應標頭 `x-pg-mode`**：沒有＝轉譯（`{r}`＝思考、`{d}`＝正文）；`passthrough`＝上游原始 chunk 直通，此時要自己解 `choices[].delta` 並呼叫 `POST /api/playground/chat/save` 落地。管理員可用 `?stream=transform` 強制舊格式 |
